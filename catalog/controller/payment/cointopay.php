...
class Cointopay extends \Opencart\System\Engine\Controller {
	public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) 
        {
            ...
            {
                $this->model_checkout_order->addHistory($php_arr->CustomerReferenceNr, 2,'Transaction #'.$php_arr->TransactionID.' Created on cointopay for OrderID #'.$php_arr->CustomerReferenceNr.' with status Processing', false);
				//print_r($php_arr);
        ...
        if(isset($_REQUEST['status']))
        {
            ...
            if(200 !== $transactionData['status_code']){
				...
				elseif($transactionData['data']['Status'] != $_REQUEST['status'] && $_REQUEST['notenough'] == 0){
					...
					{
						$statusProcessed = 15;
						$this->model_checkout_order->addHistory($_REQUEST['CustomerReferenceNr'], 1,'Cointopay Transaction #'.$_REQUEST['TransactionID'].' Set to pending (underpaid) for OrderID #'.$_REQUEST['CustomerReferenceNr'], false);
					...
					{
						$this->model_checkout_order->addHistory($_REQUEST['CustomerReferenceNr'], 14,'Cointopay Transaction #'.$_REQUEST['TransactionID'].' Set to expired for OrderID #'.$_REQUEST['CustomerReferenceNr'], false);
					...
					{
					$this->model_checkout_order->addHistory($_REQUEST['CustomerReferenceNr'], 1, 'Cointopay Transaction #'.$_REQUEST['TransactionID'].' Set to pending ('.$_REQUEST['status'].') for OrderID #'.$_REQUEST['CustomerReferenceNr'], false);
					...
					{
						$this->model_checkout_order->addHistory($_REQUEST['CustomerReferenceNr'], 7, 'Cointopay Transaction #'.$_REQUEST['TransactionID'].' Set to Canceled for OrderID #'.$_REQUEST['CustomerReferenceNr'], false);
